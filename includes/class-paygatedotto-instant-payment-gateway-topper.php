...
class PayGateDotTo_Instant_Payment_Gateway_Topper extends WC_Payment_Gateway {
    ...
    protected $topperpaycom_custom_domain;
    ...
    public function __construct() {
        ...
        $this->topperpaycom_custom_domain = rtrim(str_replace(['https://','http://'], '', sanitize_text_field($this->get_option('topperpaycom_custom_domain'))), '/');
    ...
    public function init_form_fields() {
        $this->form_fields = array(
            ...
            'topperpaycom_custom_domain' => array(
...
if (paygatedottogateway_is_checkout_block()) {
	...
	$woocommerce->cart->empty_cart();
        ...
        return array(
            'result'   => 'success',
            'redirect' => 'https://' . $this->topperpaycom_custom_domain . '/process-payment.php?address=' . $paygatedottogateway_topperpaycom_gen_addressIn . '&amount=' . (float)$paygatedottogateway_topperpaycom_final_total . '&provider=topper&email=' . $paygatedottogateway_topperpaycom_email . '&currency=' . $paygatedottogateway_topperpaycom_currency,
        );
    }
...
function paygatedottogateway_add_instant_payment_gateway_topper($gateways) {
    $gateways[] = 'PayGateDotTo_Instant_Payment_Gateway_Topper';
    return $gateways;
}
add_filter('woocommerce_payment_gateways', 'paygatedottogateway_add_instant_payment_gateway_topper');
}
