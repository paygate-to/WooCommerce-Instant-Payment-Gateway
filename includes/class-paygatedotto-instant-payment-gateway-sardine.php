...
class PayGateDotTo_Instant_Payment_Gateway_Sardine extends WC_Payment_Gateway {
    ...
    protected $sardineai_custom_domain;
    ...
    public function __construct() {
        ...
        $this->sardineai_custom_domain = rtrim(str_replace(['https://','http://'], '', sanitize_text_field($this->get_option('sardineai_custom_domain'))), '/');
...
if (paygatedottogateway_is_checkout_block()) {
	...
	$woocommerce->cart->empty_cart();
        ...
        return array(
            'result'   => 'success',
            'redirect' => 'https://' . $this->sardineai_custom_domain . '/process-payment.php?address=' . $paygatedottogateway_sardineai_gen_addressIn . '&amount=' . (float)$paygatedottogateway_sardineai_final_total . '&provider=sardine&email=' . $paygatedottogateway_sardineai_email . '&currency=' . $paygatedottogateway_sardineai_currency,
        );
    }
...
function paygatedottogateway_add_instant_payment_gateway_sardine($gateways) {
    $gateways[] = 'PayGateDotTo_Instant_Payment_Gateway_Sardine';
    return $gateways;
}
add_filter('woocommerce_payment_gateways', 'paygatedottogateway_add_instant_payment_gateway_sardine');
}
