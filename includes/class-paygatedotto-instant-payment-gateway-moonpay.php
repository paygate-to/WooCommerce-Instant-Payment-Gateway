...
if (paygatedottogateway_is_checkout_block()) {
	...
	$woocommerce->cart->empty_cart();
        ...
        return array(
            'result'   => 'success',
            'redirect' => 'https://' . $this->moonpaycom_custom_domain . '/process-payment.php?address=' . $paygatedottogateway_moonpaycom_gen_addressIn . '&amount=' . (float)$paygatedottogateway_moonpaycom_final_total . '&provider=moonpay&email=' . $paygatedottogateway_moonpaycom_email . '&currency=' . $paygatedottogateway_moonpaycom_currency,
        );
    }
...
function paygatedottogateway_add_instant_payment_gateway_moonpay($gateways) {
    $gateways[] = 'PayGateDotTo_Instant_Payment_Gateway_Moonpay';
    return $gateways;
}
add_filter('woocommerce_payment_gateways', 'paygatedottogateway_add_instant_payment_gateway_moonpay');
}
