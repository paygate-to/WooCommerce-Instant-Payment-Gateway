...
if (paygatedottogateway_is_checkout_block()) {
	...
	$woocommerce->cart->empty_cart();
        ...
        return array(
            'result'   => 'success',
            'redirect' => 'https://' . $this->banxacom_custom_domain . '/process-payment.php?address=' . $paygatedottogateway_banxacom_gen_addressIn . '&amount=' . (float)$paygatedottogateway_banxacom_final_total . '&provider=banxa&email=' . $paygatedottogateway_banxacom_email . '&currency=' . $paygatedottogateway_banxacom_currency,
        );
    }
...
function paygatedottogateway_add_instant_payment_gateway_banxa($gateways) {
    $gateways[] = 'PayGateDotTo_Instant_Payment_Gateway_Banxa';
    return $gateways;
}
add_filter('woocommerce_payment_gateways', 'paygatedottogateway_add_instant_payment_gateway_banxa');
}
